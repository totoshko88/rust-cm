...
pub struct ConnectionDialog { dialog: Dialog }

impl ConnectionDialog {
    #[must_use]
    #[allow(deprecated)]
    pub fn new(parent: Option<&Window>) -> Self {
        let dialog = Dialog::builder().title("New Connection").modal(true).default_width(500).default_height(400).build();
        if let Some(p) = parent { dialog.set_transient_for(Some(p)); }
        dialog.add_button("Cancel", ResponseType::Cancel);
        dialog.add_button("Save", ResponseType::Accept);
        Self { dialog }
    }
    pub fn set_connection(&self, _c: &Connection) { self.dialog.set_title(Some("Edit Connection")); }
    #[allow(deprecated)]
